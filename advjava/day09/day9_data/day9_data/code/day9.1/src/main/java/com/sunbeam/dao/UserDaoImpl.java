...
//			user=session.get(User.class, userId);
	...
	public String deleteUserDetailsById(Long userId) {
		...
		try {
			...
			tx.commit();
			/*
			 * session.flush() -> auto dirty checking -> DML - delete session.close() -> L1
			 * cache is destroyed and db cn rets to DBCP
			 */
			// user : transient
		} catch (RuntimeException e) {
			if (tx != null)
	...
	public String saveImage(Long userId, String filePath) throws IOException {
		...
		Transaction tx = session.beginTransaction();
		try {
			// 3. get user details by its id
			User user = session.get(User.class, userId);
			// 4 . null checking
			if (user != null) {
				// user - persistent
				// 5. validate if file exists n regular file n read permisssion
				File file = new File(filePath);
				if (file.isFile() && file.canRead()) {
					// => user n file - exists
					user.setImage(FileUtils.readFileToByteArray(file));// setter - persistent entity
					mesg = "saved image....";
				}
			}
	...
	@Override
	public String restoreImage(String email1, String filePath) throws IOException

	{
		String mesg = "restoring mesg failed....";
...
//3. get user by its email
			User user = session.createQuery(jpql, User.class)
					...
					.getSingleResult();
			// 4. => no exc => user : PERSISTENT , getter : image
			byte[] image = user.getImage();
			if (image != null) {
				// => image exists ->5. read img blob n store it in the bin file
				FileUtils.writeByteArrayToFile(new File(filePath), image);
				mesg="image restored....";
			}
			tx.commit();
		} catch (RuntimeException e) {
