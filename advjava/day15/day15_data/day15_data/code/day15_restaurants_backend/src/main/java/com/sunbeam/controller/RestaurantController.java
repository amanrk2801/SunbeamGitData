...
import com.sunbeam.Day15RestaurantsBackendApplication;
...
public class RestaurantController {
			...
			Long restaurantId)
	{
		...
		try {
			...
			return ResponseEntity
					.status(HttpStatus.NOT_FOUND) //SC 404
					.body(new ApiResponse(e.getMessage()));
		}
	}
	/*
	...
	public ResponseEntity<?> updateDetails(@PathVariable 
			Long restaurantId, @RequestBody Restaurant entity) {
		System.out.println("in update "+restaurantId+" entity");
		try {
			return ResponseEntity.ok(
					restaurantService.updateRestaurant(restaurantId, entity));
		}  catch (RuntimeException e) {
			return ResponseEntity
					.status(HttpStatus.NOT_FOUND)
					.body(new ApiResponse(e.getMessage()));
